        ...
        <div className="flex h-16 items-center justify-between gap-4">
          ...
          <nav className="hidden lg:flex items-center gap-1">
            ...
            <div className="relative group/more">
              <button className="flex items-center gap-1 px-3 py-2 text-sm font-semibold text-slate-600 rounded-lg hover:text-slate-900 hover:bg-cyan-50 transition-colors">
                More
                <ChevronDown className="h-3.5 w-3.5 transition-transform duration-200 group-hover/more:rotate-180" />
              </button>
              <div className="absolute top-full left-0 mt-2 w-48 bg-white/95 rounded-2xl border border-slate-200 shadow-xl shadow-cyan-900/10 opacity-0 invisible group-hover/more:opacity-100 group-hover/more:visible translate-y-1 group-hover/more:translate-y-0 transition-all duration-150 z-50 py-2 backdrop-blur-md">
                {moreNav.map((cat) => (
                  <Link
                    key={cat.id}
                    href={`/${cat.slug}`}
            ...
            <div className="relative group/calc">
              ...
              <div className="absolute top-full left-0 mt-2 w-56 bg-white/95 rounded-2xl border border-slate-200 shadow-xl shadow-cyan-900/10 opacity-0 invisible group-hover/calc:opacity-100 group-hover/calc:visible translate-y-1 group-hover/calc:translate-y-0 transition-all duration-150 z-50 py-2 backdrop-blur-md">
                <Link
                  href="/calculators"
